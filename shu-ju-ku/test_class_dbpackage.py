...
class TestMethod(object):
    ...
    @classmethod
    def setup_class(cls):        # 在类运行之前执行该函数   setup_class 固定的方法
    ...
    def test_update_not_exist(self, name, data):        # 定义测试更新数据库的数据不存在异常处理函数
        with pytest.raises(ValueError):
            self.db.update(name, data)


    @pytest.mark.parametrize("name", ['zhangsan', 'lisi'])
    def test_delete_exist(self, name):        # 定义测试删除数据函数
        self.db.delete(name)
        result1 = self.db.get(name)
        assert result1 == {}

    @pytest.mark.parametrize("name", ['tt','ttttt'])
    def test_delete_not_exist(self, name):        # 定义测试删除数据不存在函数
        with pytest.raises(ValueError):
            self.db.delete(name)
